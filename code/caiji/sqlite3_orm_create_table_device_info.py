...
class Router(Base):
    ...
    l2vc = relationship('L2vc', back_populates="router",
                        uselist=False, passive_deletes=True)
    arp = relationship('Arp', back_populates="router",
                       uselist=False, passive_deletes=True)
    lldp = relationship('Lldp', back_populates="router",
                        uselist=False, passive_deletes=True)
    bgp = relationship('Bgp', back_populates="router",
                       uselist=False, passive_deletes=True)
    vsi = relationship('Vsi', back_populates="router",
                       uselist=False, passive_deletes=True)
    vsi_peer = relationship(
        'Vsi_peer', back_populates="router", uselist=False, passive_deletes=True)

    vpn_instance = relationship('Vpn_instance', back_populates="router",
                                uselist=False, passive_deletes=True)
...
class L2vc(Base):
    ...
    router = relationship('Router', back_populates="l2vc",
...
class Lldp(Base):
    ...
    router = relationship('Router', back_populates="lldp",
                          passive_deletes=True)
...
class Vsi(Base):
    ...
    router_id = Column(Integer, ForeignKey(
        "router.id", ondelete='CASCADE'), nullable=False)
    VSI_Name = Column(String(64), nullable=False)
    PW_Signaling = Column(String(64), nullable=False)
    VSI_State = Column(String(64), nullable=False)
    VSI_ID = Column(String(64), nullable=False)
    Interface_Name = Column(String(64), nullable=False)
    Interface_State = Column(String(64), nullable=False)
    Ac_Block_State = Column(String(64), nullable=False)
    router = relationship('Router', back_populates="vsi", passive_deletes=True)

    def __repr__(self):
        return f"{self.__class__.__name__}(Router: {self.router.routername} " \
            f"| VSI_Name: {self.VSI_Name} " \
            f"| PW_Signaling: {self.PW_Signaling} "\
            f"| VSI_State: {self.VSI_State} " \
            f"| VSI_ID: {self.VSI_ID} " \
            f"| Interface_Name: {self.Interface_Name} "


class Vsi_peer(Base):
    __tablename__ = 'vsi_peer'
    ...
    PW_Signaling = Column(String(64), nullable=False)
    VSI_State = Column(String(64), nullable=False)
    Peer_Ip_Address = Column(String(64), nullable=False)
    PW_Last_Up_Time = Column(String(64), nullable=False)
    router = relationship(
        'Router', back_populates="vsi_peer", passive_deletes=True)

    def __repr__(self):
        return f"{self.__class__.__name__}(Router: {self.router.routername} " \
            f"| VSI_Name: {self.VSI_Name} " \
            f"| Peer_Ip_Address: {self.Interface_Name} "


class Vpn_instance(Base):
    __tablename__ = 'vpn_instance'

    id = Column(Integer, primary_key=True)
    router_id = Column(Integer, ForeignKey(
        "router.id", ondelete='CASCADE'), nullable=False)
    vpn_instance_name = Column(String(64), nullable=False)
    interfaces = Column(String(64), nullable=False)
    route_distinguisher = Column(String(64), nullable=False)
    export_vpn_targets = Column(String(64), nullable=False)
    import_vpn_targets = Column(String(64), nullable=False)
    router = relationship('Router', back_populates="vpn_instance",
                          passive_deletes=True)

    def __repr__(self):
        return f"{self.__class__.__name__}(Router: {self.router.routername} " \
            f"| vpn_instance_name: {self.vpn_instance_name} " \
